export default class MSList extends HTMLElement {
    ...
    createOrder() {
        let lastIndex = 0;
        let getIndex = (index) => {
            let meta = this.order[index] || `${this.order[lastIndex]}.${this.order[index - (this.order.length * (lastIndex + 1))]}`;
        ...
        [...this.element.children].forEach((child, index) => {
            ...
            if (!this.order)
                metadataElement.textContent = (index + 1).toString();
            else
                metadataElement.textContent = getIndex(index);
            if (!child.querySelector("ms-list-item-metadata"))
                child.appendChild(metadataElement);
        });
    }
}
